...
taytteet_pizzoissa = {}  # tyhjä sanakirja

for p in pizzat:
    for t in pizzat[p]['taytteet']:
        if t is None:
            continue
        if t in taytteet_pizzoissa:
            taytteet_pizzoissa[t] += 1
        else:
            taytteet_pizzoissa[t] = 1

print(taytteet_pizzoissa)

for k in taytteet_pizzoissa.keys():
    v = taytteet_pizzoissa[k]
    palkki = '*' * v
    print(f"{k:24} {palkki}")

print(len(taytteet_pizzoissa))

print(taytteet_pizzoissa.keys())
